...
namespace CareerGuidance.Components
{
    public partial class EgoldsFormStyle : Component
    {
        ...
        [Description("Указывает, может ли пользователь изменять размер окна при помощи кнопки")]
        public bool MaxButtonActivated { get; set; }
        ...
        private void Form_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left && Form.ControlBox == true )
            {
                ...
                // Max Button Click
                if (rectBtnMax.Contains(e.Location) && Form.MaximizeBox == true && MaxButtonActivated)
                {
                    if (Form.WindowState == FormWindowState.Maximized)
                    {
                        Form.WindowState = FormWindowState.Normal;
        ...
        {
            if (FormStyle == fStyle.None || !MaxButtonActivated) return;
